...
async def test_argtest():
    result = await run_module(load_inventory('inventory2.yml'), ['modules'], 'argtest', module_args=dict(somekey='somevalue'))
    pprint(result)
...
async def test_argtestjson():
    result = await run_module(load_inventory('inventory2.yml'), ['modules'], 'argtestjson', module_args=dict(somekey='somevalue'))
    pprint(result)
    assert result is not None
    assert 'localhost' in result
    assert result['localhost']['args']
...
async def test_timetest():
    result = await run_module(load_inventory('inventory2.yml'), ['modules'], 'timetest')
    assert result is not None
    assert 'localhost' in result
...
async def test_my_test_info():
    result = await run_module(load_inventory('inventory2.yml'), ['modules'], 'my_test_info', module_args=dict(name='foo'))
    assert result is not None
    assert 'localhost' in result
...
async def test_new_style_module():
    result = await run_module(load_inventory('inventory2.yml'), ['modules'], 'new_style_module', module_args=dict(name='foo'))
    assert result is not None
    assert 'localhost' in result
    assert 'original_message' in result['localhost']
